...
class Critter:
    ...
    def move(self, action):
        if action == "000000":
            ...
            s = "Error in display.py in def move(self, action). action ({}) not recognized.".format(action)
            sys.exit(s)
        return False
    ...
    def display_best_critter(self):
        if self.ready == False:
            if self.generation_counter >= len(self.filenames):
                ...
                sys.exit()
            print("reading file: {}".format(self.filenames[self.generation_counter]))
