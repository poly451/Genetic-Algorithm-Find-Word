...
class Critter:
    ...
    def generation(self, max_steps):
        ...
        for i in range(max_steps):
            ...
            # print("action: {}".format(myaction))
            success = self.move(myaction)
            # print(success)
            if success == True:
                print("Success!!!! You ({}) found the cheese.".format(self.id))
                self.end_x = self.x
                self.end_y = self.y
                r = True
                break
...
class Critters:
    ...
    def calculate_score(self):
        ...
        for critter in self.critters:
            ...
            if critter.get_spread() > 0:
                critter.score += round((highest_score/len(self.critters)))
    ...
    def write_to_file(self, filename):
        # utility.append_to_file(con.NEW_OUTPUT, self.print_string())
        all_critters = []
        for critter in self.critters:
...
def evolve_gen(new_gen, number_of_generations):
...
if __name__ == "__main__":
    # -------------- Clear log files ------------------------
    remove_files(os.getcwd(), "evolved_")
    # -------------------------------------------------------
    num_critters = 50
    number_of_generations = 50
    # open(con.NEW_OUTPUT, 'w').close()
    new_population = evolve.create_critters(num_critters)
    evolve_gen(new_population, number_of_generations)
