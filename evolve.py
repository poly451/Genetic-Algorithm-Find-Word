...
def get_brain():
    the_environment = ["00000", "10000", "00100", "01000", "00010", "00001"]
    the_action = []
...
def create_critters(number_of_critters):
    ...
    for i in range(number_of_critters):
        ...
        # print("colour: {}, brain: {}".format(colour, brain[0][1]))
...
def crossover(parent1, parent2):
    ...
    child[8] = utility.get_color(child[-1][0][1])
    return child
...
if __name__ == "__main__":
    critters_from_file = utility.load_json(con.NEW_OUTPUT)
    if len(critters_from_file) == 0:
        sys.exit("ERROR in evolve_population02 in evolve.py. len(critters_from_file) == 0")
    # -----------------------------------------------
    new_gen = evolve_population(critters_from_file)
    filename = utility.get_unique_filename("new_gen", ".txt")
    utility.save_json(filename, new_gen)
